...
def cell_color2(master):
    label = QLabel('2 cell color')
    inp = QPushButton()

    r,g,b,a = master.config['cell_color2']
    ...
    check_box = QCheckBox()
    check_box.setChecked(master.config['cell_hidden2'])
...
def action(inp, master):
    ...
    master.config['cell_color2'] = color.getRgb()

def cb_action(inp, master):
    master.config['cell_hidden2'] = inp.isChecked()
